...
class AuditLogRepositoryTest @Autowired constructor(
    ...
    fun `generates audit logs on rich skill update`() {
        ...
        val categoryDaos = listOf(keywordRepository.create(KeywordTypeEnum.Category, "Test Category")!!)
        ...
        val skillUpdate = RsdUpdateObject(
            ...
            authors = ListFieldUpdate(add = authorDaos),
            categories = ListFieldUpdate(add = categoryDaos),
        ...
        val secondUpdate = RsdUpdateObject(
            id = skill.id.value,
            authors = ListFieldUpdate(remove = authorDaos),
            categories = ListFieldUpdate(remove = categoryDaos),
        ...
        assertThat(secondUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::categories.name)).isEqualTo(
            Change(
                RichSkillDescriptor::categories.name,
                updatedResult?.categories?.get(0)?.value,
                null
            )
        )
